...
package object excel {
  ...
  implicit class ExcelDataFrameReader(val dataFrameReader: DataFrameReader) extends AnyVal {
    def excel(
      ...
      ).foldLeft(dataFrameReader.format("dev.mauch.spark.excel")) { case (dfReader, (key, value)) =>
        value match {
          case null => dfReader
  ...
  implicit class ExcelDataFrameWriter[T](val dataFrameWriter: DataFrameWriter[T]) extends AnyVal {
    def excel(
      ...
      Map(
        ...
        "workbookPassword" -> workbookPassword,
        "preHeader" -> preHeader
      ).foldLeft(dataFrameWriter.format("dev.mauch.spark.excel")) { case (dfWriter, (key, value)) =>
        value match {
          case null => dfWriter
